...
def parse_wikitext_to_html(wikitext):
    ...
    html_content = re.sub(
        r'href="/wiki/', r'href="https://en.wikipedia.org/wiki/', html_content
    )
    html_content = re.sub(
        r'href="/w/', r'href="https://en.wikipedia.org/w/', html_content
    )
    ...
    if os.path.exists(filename):
        try:
            with open(filename, "r") as f:
                ...
                return {
                    datetime.fromisoformat(date).date(): fetched_time
                    for date, fetched_time in data.items()
                }
...
def save_fetched_dates(fetched_dates, filename="fetched_dates.json"):
    ...
    data = {
        date.isoformat(): fetched_time for date, fetched_time in fetched_dates.items()
    }
...
def create_rss_feed(
    items, title="Wikipedia Current Events", description="Current events from Wikipedia"
):
    ...
    ET.SubElement(
        channel, "link"
    ).text = "https://en.wikipedia.org/wiki/Portal:Current_events"
...
def load_existing_rss_items(filename="current_events.rss"):
    ...
    try:
        ...
        for item in root.findall(".//item"):
            ...
            if all(
                elem is not None
                for elem in [title_elem, desc_elem, link_elem, pubdate_elem]
            ):
                items.append(
                    {
...
def prune_old_items(items, retention_days):
    """Remove items older than retention_days"""
    cutoff = datetime.now() - timedelta(days=retention_days)
    pruned = []
    for item in items:
        pubdate = parsedate_to_datetime(item["pubDate"]).replace(tzinfo=None)
        if pubdate >= cutoff:
            pruned.append(item)
    return pruned


def generate_current_events_rss(days=7, retention_days=14):
    ...
    # Prune existing items older than retention_days
    if existing_items:
        pruned_count = len(existing_items) - len(
            prune_old_items(existing_items, retention_days)
        )
        if pruned_count > 0:
            print(f"Pruned {pruned_count} items older than {retention_days} days")
        existing_items = prune_old_items(existing_items, retention_days)
    ...
    print(
        f"Need to fetch {len(dates_to_fetch)} dates: {[d.strftime('%Y-%m-%d') for d in dates_to_fetch]}"
    )
